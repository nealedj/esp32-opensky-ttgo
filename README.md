...
Under the hood it uses the [OpenSky Network](https://opensky-network.org/) API for live aircraft positions, and [AeroDataBox](https://aerodatabox.com/) (via [RapidAPI](https://rapidapi.com/aedbx-aedbx/api/aerodatabox)) to enrich flights with route and aircraft details.
Internally there is a database with airport, aircraft and airline data with graphics, and route/aircraft lookups are cached persistently, to minimize the requests.
...
This firmware offers the following features:
...
- Minimal API usage; airline/airport/aircraft databases and graphics are embedded in firmware, and route/aircraft lookups are cached persistently (LittleFS) to stay within the AeroDataBox free tier
...
When connected, the main screen is shown; the overview.
...
- The OpenSky Network API client id and client secret (create an API client under your [OpenSky account](https://opensky-network.org/)),
- The AeroDataBox RapidAPI key (subscribe to [AeroDataBox on RapidAPI](https://rapidapi.com/aedbx-aedbx/api/aerodatabox); the free tier is sufficient thanks to the persistent cache)
...
Please take a look at the STL file to create a case to host the TTGO-Display and a battery.
...
Thanks to the [OpenSky Network](https://opensky-network.org/) for the live ADS-B position data and to [AeroDataBox](https://aerodatabox.com/) for the route and aircraft enrichment API.
Thanks also to [rzeldent](https://github.com/rzeldent/esp32-flightradar24-ttgo) for the original project this is forked from.
